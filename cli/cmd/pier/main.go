...
func cmdNew(args []string) {
	...
	sess, err := drv.Create(ctx, driver.CreateSpec{
		...
		Progress: printCreateEvent,
	})
	if err != nil {
		fatal(err)
	}
	if err := config.RememberWorkspace(sess.ID, repo); err != nil {
		fmt.Fprintln(os.Stderr, ui.Bad.Render("pier:"), "remember local workspace:", err)
	}
...
const appCreateEventPrefix = "PIER_CREATE_EVENT "

func printCreateEvent(event driver.CreateEvent) {
	if os.Getenv("PIER_CREATE_EVENTS") == "json" {
		encoded, _ := json.Marshal(event)
		fmt.Println(appCreateEventPrefix + string(encoded))
		return
	}
	switch event.Kind {
	case driver.CreateSetupStarted:
		fmt.Println(ui.Step("running " + event.Message + " (output follows)"))
	case driver.CreateSetupOutput:
		fmt.Println(ui.Dim.Render("    │ " + event.Message))
	case driver.CreateSetupFinished:
		if event.ExitCode == 0 {
			fmt.Println("  " + ui.Mark(true) + " " + event.Message + " completed")
		} else {
			fmt.Println("  " + ui.Mark(false) + " " + event.Message + " failed (exit " + strconv.Itoa(event.ExitCode) + ")" +
				ui.Dim.Render(" — session kept for inspection"))
		}
	default:
		fmt.Println(ui.Step(event.Message))
	}
}
...
// cli/internal/proxy; runs in the foreground until ctrl-c.
func cmdProxy() {
