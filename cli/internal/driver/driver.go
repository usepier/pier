...
type CreateEventKind string

const (
	CreateStep          CreateEventKind = "step"
	CreateSetupStarted  CreateEventKind = "setup_started"
	CreateSetupOutput   CreateEventKind = "setup_output"
	CreateSetupFinished CreateEventKind = "setup_finished"
)

// CreateEvent is one user-visible update from a create. Setup output is kept
// distinct from ordinary milestones so terminal clients can indent it and
// native clients can render it as a live log instead of dozens of fake steps.
type CreateEvent struct {
	Kind     CreateEventKind `json:"kind"`
	Message  string          `json:"message"`
	ExitCode int             `json:"exitCode,omitempty"`
}

// CreateSpec describes a new session. Create follows .pier-setup.sh to
// completion while it runs in its own tmux window, streaming milestones and
// the script's output via Progress. A failed repo setup remains inspectable;
// infrastructure/bootstrap failures still fail and clean up the create.
