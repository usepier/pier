...
func TestSetupScriptName(t *testing.T) {
	root := t.TempDir()
	if got := setupScriptName(root, ""); got != "" {
		t.Fatalf("missing setup name = %q", got)
	}
	if err := os.WriteFile(filepath.Join(root, ".pier-setup.sh"), []byte("make build\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	if got := setupScriptName(root, ""); got != ".pier-setup.sh" {
		t.Fatalf("repo setup name = %q", got)
	}
	if got := setupScriptName(root, "/tmp/team-bootstrap.sh"); got != "team-bootstrap.sh (PIER_SETUP_SCRIPT)" {
		t.Fatalf("override setup name = %q", got)
	}
}
...
func TestPierIncludeDereferencesDirectFileSymlinks(t *testing.T) {
	...
	if misses := envFilesNotCarried(root, pierIncludeFiles(root)); slices.Contains(misses, "apps/api/.env") {
		t.Fatalf("included symlink was incorrectly reported as not carried: %v", misses)
	}
...
func TestFilesTarAlwaysCarriesRepoSetup(t *testing.T) {
	root := t.TempDir()
	const setup = "#!/usr/bin/env bash\nmake install\nmake build\n"
	if err := os.WriteFile(filepath.Join(root, ".pier-setup.sh"), []byte(setup), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	for _, tc := range []struct {
		...
		{name: "without pier include"},
		{name: "redundantly listed", include: ".pier-setup.sh\n"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			if err := os.WriteFile(filepath.Join(root, ".pier-include"), []byte(tc.include), 0o644); err != nil {
				t.Fatal(err)
			}
			dst := filepath.Join(t.TempDir(), "files.tar")
			if err := buildFilesTar(dst, nil, root, nil, ""); err != nil {
			...
			for {
				...
				if h.Mode != 0o755 || string(contents) != setup {
					t.Fatalf("setup carried as mode %o, contents %q", h.Mode, contents)
				}
			}
			if matches != 1 {
				t.Fatalf("setup tar entries = %d, want 1", matches)
			}
		})
	}
}
