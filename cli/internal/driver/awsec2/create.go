...
func (d *Driver) Create(ctx context.Context, spec driver.CreateSpec) (sess *driver.Session, retErr error) {
	...
	if progress == nil {
		progress = func(driver.CreateEvent) {}
	}
	step := func(message string) {
		progress(driver.CreateEvent{Kind: driver.CreateStep, Message: message})
	}
	...
	step(fmt.Sprintf("launched %s (%s, %s)", id, d.InstanceType, arch))
	...
	defer func() {
		...
		step("create failed — terminating the half-made instance")
		if err := d.Destroy(context.WithoutCancel(ctx), id); err != nil {
			step("cleanup failed (" + err.Error() + ") — remove it with `pier rm " + spec.Name + "`")
		}
	}()
	...
	if mode != "full" {
		...
		case sshOriginUsable(ctx, spec.Repo):
			...
			forwardAgent = true
			step("no github token — the fetch borrows your ssh agent (keys stay on this machine)")
		default:
			step("github origin not reachable with the auth sessions get — using the full bundle")
	...
	setupName := setupScriptName(spec.Repo, setupSrc)
	...
	if miss := envFilesNotCarried(spec.Repo, pierIncludeFiles(spec.Repo)); len(miss) > 0 {
		...
		step("not carrying " + name + " — env files travel only when .pier-include lists them")
	}
	...
	step("waiting for SSH over SSM (boot ~30s)")
	...
	switch mode {
	case "origin":
		step("pushing secrets — the repo comes straight from github on the VM")
	case "thin":
		step(fmt.Sprintf("pushing local-only commits (%s) — the rest comes from github", fileMB(bundle)))
		pushes = append(pushes, struct{ local, remote string }{bundle, "/tmp/pier.bundle"})
	default:
		step(fmt.Sprintf("pushing workspace (%s — full history; %s)", fileMB(bundle), why))
		pushes = append(pushes, struct{ local, remote string }{bundle, "/tmp/pier.bundle"})
	}
	if patch != "" {
		step("carrying your uncommitted edits to tracked files")
	...
	if names := OAuthRemotes(home, spec.Repo); len(names) > 0 {
		step("mcp " + strings.Join(names, ", ") + ": one-time oauth — `pier mcp login " + spec.Name + "` when it's up")
	}
	for _, p := range pushes { // biggest last: its scp meter is the wait
		if err := d.scpTo(ctx, id, p.local, p.remote); err != nil {
			...
			step("push interrupted — retrying")
	...
	step("bootstrapping (stock AMI waits for cloud-init here — `pier bake` skips that)")
	var fwd []string
	if forwardAgent {
		fwd = []string{"-A"}
	}
	...
	setupState := ""
	if setupName != "" {
		progress(driver.CreateEvent{Kind: driver.CreateSetupStarted, Message: setupName})
		exitCode, err := d.followSetup(ctx, id, func(line string) {
			progress(driver.CreateEvent{Kind: driver.CreateSetupOutput, Message: line})
		})
		if err != nil {
			return nil, fmt.Errorf("following %s: %w", setupName, err)
		}
		progress(driver.CreateEvent{Kind: driver.CreateSetupFinished, Message: setupName, ExitCode: exitCode})
		if exitCode != 0 {
			setupState = "failed"
		}
	}
...
func setupScriptName(repoRoot, override string) string {
	if override != "" {
		return filepath.Base(override) + " (PIER_SETUP_SCRIPT)"
	}
	if fi, err := os.Stat(filepath.Join(repoRoot, ".pier-setup.sh")); err == nil && fi.Mode().IsRegular() {
		return ".pier-setup.sh"
	}
	return ""
}
...
if [ -f "$setup" ]; then
  rm -f ~/.pier-setup.log ~/.pier-setup.status ~/.pier-setup.finished
  tmux new-window -d -t main -n setup "bash -c 'set -a; . ~/.config/pier/env 2>/dev/null; set +a; cd ~/work/{{REPO}} || exit 1; echo running > ~/.pier-setup.status; bash $setup 2>&1 | tee ~/.pier-setup.log; c=\${PIPESTATUS[0]}; echo \$c > ~/.pier-setup.status; if [ \$c -eq 0 ]; then echo \"pier setup: done\" >> ~/.pier-setup.log; else echo \"pier setup: FAILED (exit \$c)\" | tee -a ~/.pier-setup.log; fi; echo \$c > ~/.pier-setup.finished; if [ \$c -ne 0 ]; then tmux rename-window -t \$TMUX_PANE setup-failed; exec sleep infinity; fi'"
fi

# This bootstrap marker is the raw-SSH backstop. The client withholds the
# provider ready tag until it has followed .pier-setup.sh to completion.
...
// home/), the repo's .pier-setup.sh and files named by .pier-include (relative
// paths kept, prefix repo/), the PIER_SETUP_SCRIPT override (as
...
func buildFilesTar(dst string, manifest []string, repoRoot string, env map[string]string, setupSrc string) error {
	...
	for _, rel := range pierIncludeFiles(repoRoot) {
		// .pier-setup.sh is a Pier control file, not a loose-file opt-in. It is
		// packed unconditionally below so listing it here remains harmless and
		// does not create duplicate tar entries.
		if rel == ".pier-setup.sh" {
			continue
		}
	...
	// Always carry the current local setup script. Committed copies normally
	// arrive with the checkout and tracked edits with the dirty patch, but an
	// untracked or ignored setup script is still Pier configuration and must
	// not require a redundant .pier-include entry.
